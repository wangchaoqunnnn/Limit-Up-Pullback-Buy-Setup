...
```json
{
  ...
  "dataSourceOrder": ["eastmoney", "tencent", "ths", "sina", "yahoo"],
  "usingFallback": false,
  "sources": [
    {
      "name": "eastmoney",
...
```json
{
  ...
  "active": "tencent",
  "usingFallback": false,
  "order": ["eastmoney", "tencent", "ths", "sina", "yahoo"],
  "sources": [],
...
| `ths` | 同花顺行情接口（10jqka） |
| `sina` | 新浪财经行情接口 |
| `yahoo` | 雅虎财经 chart 接口（独立境外源，仅日线与指数） |
| `synthetic` | 内置合成演示数据（**仅当全部真实源不可用时**） |

#### 各源能力对照（均为实测结论）

| 源 | 全市场列表 | 日线 | 实时 | 指数 | 主板 | 创业板 | 科创板 | 北交所 | 备注 |
|---|---|---|---|---|---|---|---|---|---|
...
**为什么还要加一个境外源**：东方财富/腾讯/同花顺/新浪都是国内免费接口，
存在**同源风险** —— 可能因同一网络策略或同一轮反爬而一起失效。雅虎财经是
完全独立的境外服务，实测 `600519.SS` / `300750.SZ` / `688981.SS` 均返回
约 243 根日线，且**与同花顺逐字段完全一致**（O/H/L/C/V 全同）。因此把它放在
`DATA_SOURCE_ORDER` 末位：平时不参与，只在四个国内源全线不可用时兜底。
它的边界同样明确：**不提供北交所**（`920000.BJ` 返回 404），也不提供全市场列表，
所以放在最后而不是靠前 —— 避免让一个能力更弱的源承担主力取数。
...
**但「数据量不足」不能一律归咎于数据源**（实测踩坑后修正）：还有第二种成因 ——
股票**自身历史就短**（次新股）。实测 `920071`（金钛股份）2026-09-03 才上市，
四个源一致只给 7 根；`920059` 18 根、`920093` 16 根。此时任何一个源都没有错。
因此归因规则为：

| 情形 | 判定 |
|---|---|
| 某源判不足，**后续源能补齐** | 该源确有缺陷（腾讯对北交所只给 1 根）→ 记失败 |
| **所有源一致判不足** | 股票自身历史太短 → **不牵连任何源的健康度**，负缓存 20 分钟不重试 |
| 某源在**同一批次内已成功供数** | 能力正常，不因个别标的差异被降级（避免主力源被次新股拖入冷却） |
| 源返回**完全空结果** | 仍记为失败（如域名被网络阻断），该语义未被放宽 |

> 反例警示：早期实现把「数据量不足」一律记为数据源失败。由于股票池里只要有 1 只
> 次新股就会触发，一次批量即可让**四个源全部被判故障**；而「从未成功过」的源会被
> 长期跳过，于是新进程的第一次请求就把**整条源链判死**，此后连贵州茅台这样的正常
> 股票都取不到行情，且界面仍显示「演示数据」—— 属真实行情整体静默不可用。
> 该缺陷已由 `backend/tests/test_resilient.py` 中 4 个回归用例锁定。
